...
def max_mul_partition(items):
    """ O(n!)
    :param items:
    :return:
    >>> items = {'1': [40]}
    >>> max_mul_partition(items)
    Partition:  [40]  with val:  40
    >>> items = {'1': [40, 10], '2': [20, 33]}
    >>> max_mul_partition(items)
    Partition:  [40, 33]  with val:  1320
    >>> items = {'1': [40, 10, 30], '2': [15, 20, 33], '3': [32, 19, 28]}
    >>> max_mul_partition(items)
    Partition:  [40, 19, 33]  with val:  25080
    >>> items = {'1': [40, 10, 30, 31], '2': [15, 20, 33, 19], '3': [32, 19, 28, 20], '4': [32, 16, 28, 27]}
    >>> max_mul_partition(items)
    Partition:  [40, 19, 33, 27]  with val:  677160
    """
    n = len(items.keys())
    lst = []
    ...
    for l in lst:
        ...
        if sum > max_mul:
            max_mul = sum
            max_partition = l
    print("Partition: ", max_partition, " with val: ", max_mul)
